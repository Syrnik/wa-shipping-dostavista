...
const props = defineProps({
  ...
  emit = defineEmits(['update:modelValue', 'delete']),
  interval = ref(props.modelValue);
...
<template>
  <tr>
    ...
    <td v-for="v in [1,2,3,4,5,6,7]"><input type="checkbox" :name="addns('', addns('day', ns))" :value="v"
                                            v-model="interval.day" @change="emitChanges"></td>
    ...
    <td><input type="checkbox" :name="addns('workday', ns)" v-model="interval.workday" @change="emitChanges"></td>
      <td><a class="inline-link"
             @click.prevent="$emit('delete')"><i class="icon16 delete fa fa-trash-alt text-red"></i></a></td>
  </tr>
</template>
