...
const props = withDefaults(defineProps<{ vars?: string[] }>(), {
  vars: () => ['z', 'y', 's'],
})

const count_txt: Record<number, string> = {
  1: 'доступна одна переменная',
  2: 'доступны две переменные',
  3: 'доступны три переменные',
}

const formula_desc = ref<HTMLElement | null>(null)

function openDrawer() {
  ($ as any).waDrawer({
    html: [].map.call(formula_desc.value!.children, (e: Element) => e.outerHTML).join('\n'),
    direction: 'right',
  })
}
</script>
        ...
        <div class="drawer-block w-syrnik-shipping-ui">
          ...
          <div class="drawer-content">
            ...
            <p>Можно использовать математические символы (сложение, деление, вычитание, умножение и т.д.),
              скобки <span class="hint">(квадратный корень, синус, косинус тоже можно)</span>.</p>
            ...
            <dl class="w-syrnik-shipping-terms">
              ...
              <dt v-if="vars.includes('s')"><code class="semibold highlighted green">Z*0.025+S+100</code></dt>
              <dd v-if="vars.includes('s')">2.5% от суммы заказа (Z*0.025) плюс расчетная стоимость доставки плюс ещё 100 рублей</dd>
              <dt v-if="vars.includes('s')"><code class="semibold highlighted green">Z*0.1+S^2</code></dt>
              <dd v-if="vars.includes('s')">10% от суммы заказа плюс квадрат стоимости доставки (доставка в степени 2) 🙃</dd>
              <dt v-if="vars.includes('y')"><code class="semibold highlighted green">max(1000, Y)</code></dt>
              <dd v-if="vars.includes('y')">стоимость заказа без учёта скидок, но не менее 1000&nbsp;₽</dd>
              <dt v-if="vars.includes('z')"><code class="semibold highlighted green">min(5000, Z)</code></dt>
